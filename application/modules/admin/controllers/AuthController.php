				...
				if($this->getRequest()->isPost())
				{
					if (isset($params['username']) && isset($params['password']) && isset($params['email']))
					{
						$q_user = Doctrine_Query::create()
															->select()
															->from("Users usr")
															->where("usr.usr_username = ?", $params['username'])
															->orWhere("usr.usr_email = ?", $params['email']);
						$a_user = $q_user->fetchArray();
						if(count($a_user) == 0)
						{
							$user = new Users();
							$user->usr_username = $params['username'];
							$user->usr_password = password_hash($params['password'], PASSWORD_DEFAULT);
