...
class CU_Form_Model extends Zend_Form
{
	...
	/**
	 * store field names identity
	 * @var unknown_type
	 */
	public $_fieldNames = array();
	...
	{		
		...
		elseif($this->_adapter == null)
			require_once 'CU/Form/Model/Adapter/Doctrine.php';
	...
	public function setRecord($instance)
	{
		...
		foreach($this->_adapter->getColumns() as $name => $definition)
		{
			...
			if($definition['type']=='date' || $definition['type']=='timestamp'){
				if($value!='' && $value!='9999-12-31' && $value!='1000-01-01'){
					$value = date('d-m-Y',strtotime($value));
				}elseif($value=='9999-12-31' || $value=='1000-01-01'){
					$value = '';
				}
				
			}	
			$this->setDefault($this->getColumnElementName($name),$value);
		}
//
...
//		}
	...
	protected function _generateForm()
	{
		...
		//disable relation field because we have really complex	relation tables
	...
	protected function _columnsToFields()
	{
		foreach($this->_adapter->getColumns() as $name => $definition)
		{
			...
			if($type=='timestamp' || $type=='date'){
				$field = new ZendX_JQuery_Form_Element_DatePicker($this->_fieldPrefix.$name);
				$field->setJQueryParam('dateFormat','dd-mm-yy'	);
				$field = $this->createElement('text', $this->getColumnElementName($name));
				$field->setAttrib	 ('class','form_datepicker ui-corner-all');
        		
			}else{
				$field = $this->createElement($type, $this->getColumnElementName($name));
				$field->setAttrib('class','ui-corner-all');
			}
			$this->_fieldNames[] = $this->_fieldPrefix.$name;
			$label = $name;
...
//			set auto required off (we do not want to fill everything for now) :)			
...
//				break;
	...
	public function save($persist = true)
	{
		...
		foreach($this->_adapter->getColumns() as $name => $definition)
		{
			...
			$value = $this->getValue($this->getColumnElementName($name));
			if($definition['type']=='date' || $definition['type']=='timestamp'){
